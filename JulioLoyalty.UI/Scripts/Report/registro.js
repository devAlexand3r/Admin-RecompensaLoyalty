$(function (e) {
    ...
    reporte.urlAPI = {
        url_reporte_registro: appBaseUrl + '/api/Report/operativo',
        url_consultar_registro_detalle: appBaseUrl + '/api/Report/operativo_detalle',
    };
    ...
    reporte.webControls = {
        ...
        table_registro_detalle: $('#table_registro_detalle'),
        ...
        reporte.webControls.table_reporte_dinamico.on('click', 'td', function (event) {
            ...
            if (selValue > 0) {
                ...
                if (event_tarjet === 'label label-default') {
                    ...
                    reporte.consultaRegistroDetalle(colIndex, trid);
    ...
    reporte.obtenerReporteDinamico = function () {
        if (reporte.valido.fecha === true) {
            ...
            var dataResult = [];
            var rcolumns = [];
            $.getJSON(this.urlAPI.url_reporte_registro,
                {
                    tipo_reporte_id: 1,
                ...
                function (json) {
                    ...
                    if (array.length > 0) {
                        ...
                        $.each(types, function (index, name) {
                            ...
                            if (name.toUpperCase() === "CON CORREO" || name.toUpperCase() === "SIN CORREO" || name.toUpperCase() === "TOTAL") {
                                var controles = {
                                    sTitle: name,
                                    mData: name,
                    ...
                    var conf = {
                        ...
                        customize: function (xlsx, row) {
                            ...
                            $('row c[r^="B"],row c[r^="C"],row c[r^="D"]', sheet).attr('s', 63);
    ...
    reporte.consultaRegistroDetalle = function (colIndex, id) {
        console.log("Peticion Detalle");
        var dataResult = [];
        var rcolumns = [];
        $.getJSON(reporte.urlAPI.url_consultar_registro_detalle, { tipo_reporte_id: 1, fecha_inicial: $('#fec_ini').val(), fecha_final: $('#fec_fin').val(), campo_id: colIndex, distribuidor_id: id }, function (json) {
            var array = JSON.parse(json);
            if (array.length > 0) {
                const types = Object.keys(array[0]);
                var columnDefault = [];
                ...
                if ($.fn.DataTable.isDataTable(reporte.webControls.table_registro_detalle)) {
            ...
            reporte.table_detalle = reporte.webControls.table_registro_detalle.DataTable(dataResult[0]);
        })
    };
