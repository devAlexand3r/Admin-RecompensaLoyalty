$(function (e) {
    ...
    reporte.urlAPI = {
        url_reporte_puntos: appBaseUrl + '/api/Report/operativo'
    };
    ...
    reporte.obtenerReporteDinamico = function () {
        if (reporte.valido.fecha === true) {
            ...
            var dataResult = [];
            var rcolumns = [];
            $.getJSON(this.urlAPI.url_reporte_puntos,
                {
                    tipo_reporte_id: 6,
                ...
                function (json) {
                    ...
                    if (array.length > 0) {
                        ...
                        $.each(types, function (index, name) {
                            ...
                            if (name.toUpperCase() == "SUCURSAL") {
                                const add = { sTitle: name.replace('_', ' '), mData: name }
                                rcolumns.push(add);
                            }
                            else {
                                const add = {
                                    sTitle: reporte.maysPrimera(name).replace('_', ' '),
                                    mData: name,
                    ...
                    var conf = {
                        ...
                        customize: function (xlsx, row) {
                            ...
                            $('row c[r^="B"],row c[r^="C"]', sheet).attr('s', 63);
